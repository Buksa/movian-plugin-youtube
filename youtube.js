...
var REGION = 'us';
var PREFIX = "youtube";
var UA = 'Mozilla/5.0 (Windows NT 6.1; rv:50.0) Gecko/20100101 Firefox/50.0';
...
var videoId = require('showtime/store').create('videos');//[];
var playlists = [];
var mychannelPos = 0;
var mychannelTitle = "My Channel";
var mychannelIcon = null;

function pagemeta(page, endpoint, query, block, channelid) {
  query.part = 'snippet';
  if(endpoint === 'channels') query.part += ",brandingSettings";

  if(channelid && channels[channelid])
	{
		if(channels[channelid].background)
		{
			page.metadata.background = channels[channelid].background;
...
function populate(info)
{
    ...
    if(item.snippet.thumbnails && item.snippet.thumbnails.medium)
	{
	  item.snippet.thumbnails.medium.url = item.snippet.thumbnails.medium.url.replace(/\/default.jpg/g,"/mqdefault.jpg")
	...
	if(endpoint === 'channels' && item.brandingSettings && item.brandingSettings.image.bannerTvLowImageUrl)
    {
	...
	else
	  {
		channels[query.id] = {
			title: item.snippet.title,
			icon: item.snippet.thumbnails.medium.url, //(item.snippet.thumbnails?(item.snippet.thumbnails.medium?item.snippet.thumbnails.medium.url:item.snippet.thumbnails.default.url):null),
			background: null,
...
function playlistPage(page, playlistid, channelid) {
  pagemeta(page, 'playlists', {id: playlistid}, false, channelid);
...
function videoPage(page, id) {
  ...
  page.type = 'video';
	...
	if(videoId[id] && (parseInt(Date.now()/1000) - videoId[id].date) < 21300) {
		info = videoId[id];
	}
	...
	if(info && info.url)
	{
		var videoParams = {
			...
			icon: 'http://i.ytimg.com/vi/'+id+'/mqdefault.jpg', //info.thumb,
